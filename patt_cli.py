...
if __name__ == "__main__":
    ...
    cli.add_argument('-r','--postgres_release', help='postgres release version 11|12', default="11", required=False)
    cli.add_argument('-pp','--postgres_parameters', help='list of configuration settings for Postgres', default=[""], required=False)

    cli.add_argument('--patroni_release', help='patroni release version', default="1.6.5", required=False)
    ...
    cli.add_argument('--add-repo', help="add (and enable) the repo from the specified URL", action='append', required=False,
                        default=["http://[2a00:1098:25:171::16]/centos/testing/8/"])
    ...
    logger.info ("postgres_peers : {}".format([(n.hostname, n.id, n.ip_aliases) for n in postgres_peers]))
